...
<div class="mainContainer">
    ...
    <div class="contentContainer">
        ...
        <div class="right">
            <h3 style="border-bottom: 1px solid #68a54b;width: 500px">Enter COT to continue with transfer...</h3>
            <p>Please provide a COT Code given to you by the Bank to continue with your transfer. Please contact your bank if you do not have it.</p>
            <?php
				if(isset($_POST['cot_code'])){
				$cot_code = $_POST['cot_code'];
				if(!empty($cot_code)){
				$query = mysql_query("SELECT `codes` FROM `codes` WHERE `codes` = '$cot_code' AND `code_type` ='COT Code'");
				$checkRow = mysql_num_rows($query);
				if($checkRow > 0){
					echo "<div class=\"alert alert-success\">" . 'COT Code Successfully verified.' . ' Your transfer would coninue' . "</div>";
					echo '<br />';
					echo '<p>'.'You are been redirected to continue with your transfer in 5 seconds '.'</p>';
					header('refresh: 5, url=transfering_in_progress.php');
				}else{
				echo "<div class=\"alert alert-danger\">";
				echo 'Invalid COT CODE, Please try again';
				echo "</div>";
				}
				}else{
				echo "<div class=\"alert alert-danger\">";
				echo 'Please enter a COT Code';
				echo "</div>";	
				}
				}
